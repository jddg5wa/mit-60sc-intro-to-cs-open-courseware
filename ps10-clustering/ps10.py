...
def test(points, k = 200, cutoff = 0.1):
    ...
    for i in range(len(clusters)):
        ...
        incomes.append(getAvgIncome(clusters[i]))

    # for num in range(0,len(clusters)):
    #     print clusters[num].points

    pylab.hist(incomes)
    pylab.xlabel('Avg. Income')
...
def graphRemovedErr(points, kvals = [25, 50, 75, 100, 125, 150], cutoff = 0.1):
    ...
    """
    partitionPercents = [.8, .2] #"percentages" in decimal form.
    trainingErrors = []
    holdoutErrors = []

    if sum(partitionPercents) != 1.0:
        raise ValueError("Partion percents do not add up to 100.")

    for k in kvals:
        random.shuffle(points)
        trainingSet = points[0:int(math.floor((partitionPercents[0]*len(points))))]
        holdoutSet = points[int(math.ceil((partitionPercents[0]*len(points)))):len(points)]

        clusters, maxDist = kmeans(trainingSet, k, cutoff, County)
        ...
        for cluster in clusters:
            ...
            for point in clusterPoints:
                trainingError += point.distance(centroid)**2

        trainingErrors.append(trainingError)
        print "\n" + "training error = " + str(trainingError)

        for point in holdoutSet:
            holdoutError = 0
            nearestCluster = []
            for cluster in clusters:
                if nearestCluster != [] and point.distance(cluster.getCentroid()) >= point.distance(nearestCluster.getCentroid()):
                    nearestCluster = cluster
                else:
                    nearestCluster = cluster
            holdoutError += point.distance(nearestCluster.getCentroid())**2

        holdoutErrors.append(holdoutError)        
        print "holdout error = " + str(holdoutError) + "\n"

    pylab.plot(kvals, trainingErrors, 'g.--')
    pylab.plot(kvals, holdoutErrors, 'b.--')
    pylab.legend(("training set", "holdout set"), loc='upper right')
    pylab.xlabel('Amount of Initial Centroids')
